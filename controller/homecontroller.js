...
module.exports.createTodo = function(req,res){
   ...
   let newdate='';
    ...
    TodoLists.create({
        ...
        dueDate: newdate,
        // Backg:Background[req.body.category]
...
module.exports.EditPage = function(req,res){ // here we are fetching the data which need to be edited
    ...
    TodoLists.findById(req.query.id,function(err,todoLists){
        if(err){ console.log('hi man!! it an error');
         return}
...
module.exports.getBoard = function (req, res) {
    TodoLists.distinct("category", function (err, categories) {
        if (err) {
            console.error(err);
            return res.status(500).send("Internal Server Error");
        }
        
       const totalDistinctCategories = categories.length;

        return res.render('board', {
            title: "Home",
            categories: categories,
            totalDistinctCategories: totalDistinctCategories // Pass the total count to the template
        });
    });
};